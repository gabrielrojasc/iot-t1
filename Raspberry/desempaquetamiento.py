...
def prot_unpack(protocol:int, data):
    ...
    if protocol == 4:
        array = (*array[:16], array[16:16+2000], array[16+2000:16+4000], array[16+4000:])
    return array

def header_dict(data):
    id_device, M1, M2, M3, M4, M5, M6, transport_layer, protocol, leng_msg = unpack("<H6B2BH", data)
    MAC = ".".join([hex(x)[2:] for x in [M1, M2, M3, M4, M5, M6]])
    return {"id_device":id_device, "MAC":MAC, "transport_layer":transport_layer, "protocol":protocol, "length":leng_msg}
...
def data_dict(protocol:int, data):
    ...
    p1 = ["Batt_level", "Timestamp", "Temp", "Press", "Hum", "Co"]
    p2 = ["Batt_level", "Timestamp", "Temp", "Press", "Hum", "Co", "RMS"]
    p3 = ["Batt_level", "Timestamp", "Temp", "Press", "Hum", "Co", "RMS", "Amp_x", "Frec_x", "Amp_y", "Frec_y", "Amp_z", "Frec_z"]
    p4 = ["Batt_level", "Timestamp", "Temp", "Press", "Hum", "Co", "Acc_x", "Acc_y", "Acc_z"]
    p = [p0, p1, p2, p3, p4]
